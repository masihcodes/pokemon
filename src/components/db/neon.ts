...
export async function getUserByCookie(): Promise<Partial<UserType> | null> {
  const cookie = await cookies();
  const id = cookie.get("user_token")?.value;
  if (!id) return null;
  const [res] = await db
    .select({ id: users.id, name: users.name, email: users.email }).from(users).where(eq(users.id, Number(id))).limit(1);
